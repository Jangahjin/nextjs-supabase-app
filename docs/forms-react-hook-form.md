...
> ⚠️ **현재 이 저장소에는 `react-hook-form`, `@hookform/resolvers`, `zod`, `use-debounce`, `react-error-boundary` 중 어느 것도 설치되어 있지 않습니다** (`package.json` 기준). 아래는 도입을 원할 때 따를 수 있는 권장 패턴이며, 이 저장소의 현재 구현 상태가 아닙니다. 실제로 지금 존재하는 인증 폼(`components/login-form.tsx`, `components/sign-up-form.tsx`, `components/forgot-password-form.tsx`, `components/update-password-form.tsx`)이 어떻게 만들어져 있는지는 이 문서 맨 아래 "현재 저장소의 실제 폼 패턴" 섹션을 먼저 확인하세요.
...
```bash
# 이 패턴을 도입하려면 먼저 설치가 필요함 (현재 미설치)
npm install react-hook-form @hookform/resolvers zod

# 고급 기능을 위한 추가 패키지 (선택적, 역시 미설치)
...
이 예시는 `components/ui/form.tsx`(shadcn의 Form/FormField 래퍼, `npx shadcn@latest add form`으로 추가 가능하며 내부적으로 react-hook-form에 의존)가 존재한다고 가정합니다. 현재 `components/ui/`에는 `badge`, `button`, `card`, `checkbox`, `dropdown-menu`, `input`, `label`만 있고 `form.tsx`는 없으므로, 이 패턴을 쓰려면 먼저 `npx shadcn@latest add form`을 실행해야 합니다. 파일 경로도 실제 컨벤션에 맞춰 `components/login-form.tsx`(하위 `forms/` 폴더 없이 평평한 구조)로 둡니다.
...
- [ ] `npx tsc --noEmit` 통과 (전용 `typecheck` 스크립트 없음)
- [ ] `npm run lint` 통과
- [ ] TypeScript 엄격 모드 오류 없음
- [ ] 불필요한 리렌더링 없음
- [ ] 메모리 누수 없음 (useEffect cleanup)

> Prettier는 이 프로젝트에 설치되어 있지 않으므로 포맷팅 체크 항목은 해당 없음.
```
...
위의 모든 내용은 도입을 원할 때의 권장 패턴이며, **현재 이 저장소의 인증 폼은 위 패턴을 전혀 사용하지 않습니다.** `components/login-form.tsx`, `components/sign-up-form.tsx`, `components/forgot-password-form.tsx`, `components/update-password-form.tsx`는 모두 다음과 같은 훨씬 단순한 패턴으로 되어 있습니다:
...
export function LoginForm() {
  ...
  return (
    <form onSubmit={handleLogin}>
      {/* Input, Label, Button 등 components/ui/* 사용 */}
    </form>
  );
}
```
...
즉 현재 구현은:
- `useState`로 필드값·에러·로딩 상태를 직접 관리 (React Hook Form 없음)
- 별도 검증 스키마 없이 HTML `required` 속성 + Supabase 자체 에러 메시지에 의존 (Zod 없음)
- Server Action이 아니라 **Client Component에서 `@/lib/supabase/client`를 통해 Supabase Auth API(`signInWithPassword`, `signUp` 등)를 직접 호출** (`app/actions/auth.ts` 같은 Server Action 파일 자체가 존재하지 않음)
- 성공 시 `next/navigation`의 `useRouter().push()`로 이동 (`redirect()` 아님)

새 인증 관련 폼을 추가할 때는 기존 4개 폼과의 일관성을 위해 우선 이 실제 패턴을 따르는 것을 권장합니다. React Hook Form + Zod + Server Actions 패턴으로 전환하려면 위 가이드를 참고하되, 그 경우 기존 4개 폼도 함께 마이그레이션할지 결정이 필요합니다(패턴 혼재 방지).
